...
GCS_FILES = ['create_op.sql', 'create_au.sql', 'create_cg.sql', 'create_ce.sql']
PREFIX = 'sql'
...
sql_query = "{{ task_instance.xcom_pull(task_ids='read_sql_bq') }}"
...
with DAG(
    ...
    read_sql_bq = PythonOperator(
            task_id='read_sql_bq',
            provide_context=True,
            python_callable=read_gcs_sql,
            )

    execute_sql_bq = PythonOperator(
        task_id='execute_sql_bq',
        provide_context=True,
...
start_task >> gcs_sensor  >> load_data >> read_sql_bq >> execute_sql_bq >> move_file >> end_task